...
const fixtures = load('../fixtures/resources/cases.json');
assert.equal(fixtures.length, 64, 'all 64 resource fixtures are covered');

for (const fixture of fixtures) {
  test(`Phase 4 conformance: ${fixture.id}`, () => {
...
function checkMode(fixture: FixtureCase, strict: boolean): void {
  ...
  assert.equal(result.status === 'ok', expected.valid, `${fixture.id}: ${strict ? 'strict' : 'normal'} validity`);
  if (result.status === 'ok') {
...
function load(relative: string): FixtureCase[] {
  return (JSON.parse(readFileSync(new URL(relative, import.meta.url), 'utf8')) as { cases: FixtureCase[] }).cases;
}
